  ...
  if (messages.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center text-white/50 h-full">
        ...
        <p className="mt-2 text-sm">NOVA Autonomous Agent is ready.</p>
      </div>
    );
  }

  return (
    <div className="flex-1 overflow-y-auto px-4 py-8 space-y-8 scroll-smooth h-full">
      {messages.map((msg, index) => (
        <div key={msg.id} className={clsx("flex gap-4 max-w-4xl mx-auto", msg.role === 'user' ? "flex-row-reverse" : "flex-row")}>
          ...
          <div className={clsx("flex flex-col gap-1 max-w-[80%]", msg.role === 'user' ? "items-end" : "items-start")}>
            ...
            <div className={clsx(
              "px-5 py-3 rounded-2xl text-sm leading-relaxed text-white/90 shadow-lg",
              msg.role === 'user' 
                ? "bg-blue-600 rounded-tr-sm" 
                : "bg-white/10 backdrop-blur-md border border-white/5 rounded-tl-sm"
            )}>
              {msg.role === 'user' ? (
